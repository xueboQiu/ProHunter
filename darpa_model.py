...
class GINConv(MessagePassing):
    ...
    def __init__(self,node_dim, edge_dim, emb_dim, aggr="add", input_layer=False):
        ...
        self.edge_encoder = torch.nn.Linear(self.edge_dim, emb_dim)
...
class GNN(torch.nn.Module):
    ...
    def __init__(self,node_dim, edge_dim, num_layer, emb_dim, JK="last", drop_ratio=0, gnn_type="gin"):
        ...
        for layer in range(num_layer):
            ...
            if gnn_type == "gin":
                self.gnns.append(GINConv(node_dim,edge_dim, emb_dim, aggr="add", input_layer=input_layer))
            elif gnn_type == "gcn":
                self.gnns.append(GCNConv(node_dim,edge_dim, emb_dim, input_layer=input_layer))
            elif gnn_type == "gat":
                self.gnns.append(GATConv(node_dim,edge_dim, emb_dim, input_layer=input_layer))
