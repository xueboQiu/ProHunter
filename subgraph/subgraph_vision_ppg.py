...
def trans2nx(lines,uuid2name,uuid2type):
    ...
    for i,l in enumerate(lines):
        ...
        src, edge_type, dst = l.strip().split(" ")
...
def attach_attributes(g,poi_uuid=""):
    ...
    for node in g.nodes():
        ...
        elif "Flow" in node_type or "FLOW" in node_type:
...
def draw_graph(vis_graph,dataset,host=""):
    ...
    from subgraph.subgraph_extraction import get_subgraphs_attributes
...
if __name__ == '__main__':
    ...
    for dataset in datasets:
        ...
        in_dir = "../dataset/{}/sgs".format(dataset)
        # list the files in the directory
        import os
        files = os.listdir(in_dir)

        # get groundtruth files
        gt_paths = [f"../dataset/{dataset}/groundtruth/{f}" for f in os.listdir(f"../dataset/{dataset}/groundtruth")]

        for f in files:

            if "system0051" not in f: continue

            poi = f.split('_')[-1].split('.txt')[0]
            ...
            if "optc" in dataset:
                ...
                sce = f.split('_')[0]
            file_path = f'../dataset/{dataset}/sampled_graphs/sg_{sce}_{poi}.json'
            # with open(file_path, 'w') as f:
            #     json.dump(json_data, f, indent=4)

            draw_graph(nx_muldi_graph,dataset,sce)

            # gt_path = [p for p in gt_paths if poi in p][0]
            # gt_graph = read_json_graph(gt_path)
            # draw_graph(gt_graph,dataset,sce)