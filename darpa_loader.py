...
class DarpaDataset4MEGRAPT(InMemoryDataset):
    def __init__(self,
                 root,
                 data_type,
                 empty=False,
        ...
        super(DarpaDataset4MEGRAPT, self).__init__(root, transform, pre_transform, pre_filter)
        if not empty:
            data_list = torch.load(self.processed_paths[0])
            # Convert ProHunter dataset to MEGRAPT compatible dataset
            data_list = util.compat2MEGRAPT(data_list, is_train=True)
